...
const handleKeyPress = (event) => {
  ...
  const handlePriceFilterChange = (event) => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/admin/products?category=Cracked`);
        const data = await response.json();
...
<h3>
  ...
  <Link to="/cracked" className="breadcrumb-link">Cracked</Link>
