...
const handleKeyPress = (event) => {
  ...
  const handlePriceFilterChange = (event) => {
    ...
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/admin/products?category=International`);
        const data = await response.json();
...
<h3>
  ...
  <Link to="/international" className="breadcrumb-link">International</Link>
